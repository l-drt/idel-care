...
| **Audit** | Modèle Prisma `AuditLog`, **AuditService**. Auth : `LOGIN_*`, `LOGOUT` (IP, userAgent). **Patients** : `PATIENT_CREATE`, `PATIENT_LIST`, `PATIENT_VIEW`, `PATIENT_SOFT_DELETE` (IP, userAgent). | Conservation 3 ans = politique de rétention (infra / procédure). Voir [DEPLOIEMENT.md](../DEPLOIEMENT.md). |
| **RGPD** | Modèle `Patient` : `consentGiven`, `consentDate`, `deletedAt`. Consentement obligatoire à la création (API + app). Soft delete : DELETE /patients/:id, filtrage `deletedAt` en liste/détail. Paramètres app : Données collectées, Exporter mes données, Supprimer mon compte. | **Portabilité patient** : endpoint d’export (JSON/PDF) à ajouter. **Anonymisation** (optionnel) : après soft delete si besoin. **Minimisation** : respectée par le modèle et les écrans. |
...
En résumé : **connexion (2FA, JWT, bcrypt)** = bon. **Audit** : auth + patients en place. **RGPD patients** : consentement obligatoire et soft delete en place ; portabilité (export) à ajouter. **Infrastructure** = à mettre en place au déploiement (hébergeur HDS, TLS, sauvegardes, PRA).
